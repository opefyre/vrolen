...
- **Optimization search**: 3D grid sweeps buffer × cycle-multiplier × tool-pool delta; ranks candidates by 6 objectives (throughput, time-in-system, OEE, good-parts/h, WIP, energy / part) with a heatmap + Pareto-frontier scatter that swaps its X axis to match the active objective. Three feasibility constraints — max time-in-system, max average WIP, max energy / part — strike through cells that violate them and exclude infeasible candidates from the winner pick.
