...
- **Optimization search**: 3D grid sweeps buffer × cycle-multiplier × tool-pool delta; ranks candidates by 6 objectives (throughput, time-in-system, OEE, good-parts/h, WIP, energy / part) with a heatmap + Pareto-frontier scatter that swaps its X axis to match the active objective. Three feasibility constraints — max time-in-system, max average WIP, max energy / part — strike through cells that violate them and exclude infeasible candidates from the winner pick. Each candidate also carries a 95 % CI on throughput (Bessel-corrected sample stddev × z=1.96 / √n); the picker prefers the higher LOWER bound when CIs overlap so the "best" is the statistically robust pick, not just the highest mean.
